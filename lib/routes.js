module.exports = function(app) {
  ...
  let updateCurrentMysqlConnection = function(data, request) {
    Object.assign(currentMysqlConnection, data);
    return currentMysqlConnection;
  };

  let updateCurrentPostgresConnection = function(data, request) {
    request.session.postgresCredentials = data.remember ? data : {};
  ...
  app.get("/", function(req, res) {
    let params = {
      type: req.query.type || "postgres",
      postgresError: req.query.postgresError || 0,
      mysqlError: req.query.mysqlError || 0,
      postgresCredentials: req.session.postgresCredentials || {},
      mysqlCredentials: req.session.mysqlCredentials || {}
    };
  ...
  app.post("/postgresql", function(req, res) {
    postgresqlController.establishConnection(
      updateCurrentPostgresConnection(req.body, req),
      function(data, err) {
        if (err) {
  ...
  app.get("/postgresql", function(req, res) {
    if (postgresqlController.checkConnection()) {
      res.render("postgresql");
    } else {
      res.redirect("/");
    }
