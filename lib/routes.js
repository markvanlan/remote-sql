module.exports = function(app){
  ...
  app.get('/mysql/:database/table/:table/:offset/:count/:sortColumn/:sortType', function(req, res) {
    mysqlController.getTableDetails(req.params.table ,function(details, err1){
      if (err1)
        res.send(err1)  
      else {
        mysqlController.getTableData(req.params.table, req.params.offset, req.params.count, req.params.sortColumn, req.params.sortType ,function(data, err2){
          if (err2)
            res.send(err2)  
          else {
