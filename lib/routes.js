module.exports = function(app){
  ...
  let postgresController = require("./postgres.js");
  ...
  app.get('/mysql/:database/table/:table', function(req, res) {
    mysqlController.getTableDetails(currentMysqlConnection,req.params.database, req.params.table ,function(details, err1){
      if (err1)
        res.send(err1)  
      else {
        mysqlController.getTableData(currentMysqlConnection,req.params.database, req.params.table ,function(data, err2){
        if (err2)
          res.send(err2)  
        else {
          res.send([details,data])
